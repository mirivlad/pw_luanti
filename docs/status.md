...
| A town wall is a rectangle and is built straight across water | The perimeter is a rectangle eight nodes outside the settlement bounds, and the wall is founded on `paving_level`, which in a flooded column returns the floor of the basin | Photographed from the air, Scorchmarket's wall marches in a straight line out into shallow sea. It is the most artificial thing in the world as it stands. An attempt to skip flooded cells broke a neighbouring test in a way I could not explain inside a reasonable budget and was reverted rather than committed half-working |
