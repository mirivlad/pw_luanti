...
| A town can be sited on a shoal | The survey radius was fixed at 24 whatever was to be built, so a town 167 nodes wide was judged on a patch 48 across — it reported water 0% and buildable 99%. The radius now follows the settlement type, up to 48, which is as far as `emerge_village_area` generates | Fixed for new settlements. Scorchmarket was built under the old rule and is still standing in the water, joined by boardwalks |
